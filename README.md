...
This project grew in phases. Each one was chosen to close a specific gap between "I know how to run this" and "I know why this is the right pattern."

### Foundation: bare metal → virtualised estate

Starting point: a single Proxmox host with VMs provisioned manually through the GUI. Network DNS and DHCP were on the Virgin Media hub — no control, no filtering, no local resolution. The NAS was a stripe pool with a faulted drive.
...
- Tofu state remote backend (S3-compatible via Minio on the NAS, or Terraform Cloud free tier) so state survives an JBVM02 rebuild
- Kustomize overlay structure exploration — understanding the base/overlay pattern before needing it in production
- Network policy: Kubernetes NetworkPolicy resources to restrict pod-to-pod traffic within the cluster
